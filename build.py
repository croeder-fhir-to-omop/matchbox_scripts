...
_IG_RESTORE_FILE = IG_DIR / '.ig_checkout_restore'


def _ig_recover_if_needed():
    """If a previous run was killed mid-checkout, restore fhir-omop-ig to the saved branch."""
    if _IG_RESTORE_FILE.exists():
        saved = _IG_RESTORE_FILE.read_text().strip()
        print(f'WARNING: previous build was interrupted mid-checkout. Restoring fhir-omop-ig to: {saved}')
        subprocess.run(['git', 'checkout', saved], cwd=str(IG_DIR), check=False)
        _IG_RESTORE_FILE.unlink()
...
def _ig_source_checkout():
    ...
    # Write restore file before checkout so a SIGKILL mid-build can be recovered on next run.
    _IG_RESTORE_FILE.write_text(orig or 'main')
...
def _resolve_run(source: str | None, include_test: bool) -> list:
    ...
    if source == 'local':
        ig_dirty      = _detect_dirty(IG_DIR, exclude=['FHIR-omop.xml', '.gitignore'])
