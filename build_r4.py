...
# fhir-omop-ig branch the ig step builds from. Checked out explicitly (see
# _ig_source_checkout below) rather than relying on whatever's already checked out.
_IG_SOURCE_BRANCH = 'main'
...
def _ig_source_checkout():
    """Check out _IG_SOURCE_BRANCH in fhir-omop-ig, then restore the original branch."""
    ...
    print(f'\n>>> git checkout {_IG_SOURCE_BRANCH}  (in fhir-omop-ig)')
    subprocess.run(['git', 'checkout', _IG_SOURCE_BRANCH], cwd=str(IG_DIR), check=True)

    commit = subprocess.run(
        ['git', 'rev-parse', 'HEAD'], capture_output=True, text=True, cwd=str(IG_DIR)
    ).stdout.strip()
    print(f'>>> fhir-omop-ig at {commit[:8]} ({_IG_SOURCE_BRANCH})')
