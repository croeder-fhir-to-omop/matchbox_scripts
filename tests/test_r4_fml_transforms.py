...
Requires a running R4 matchbox server (MATCHBOX_URL env var or default http://localhost:8080).

Mirrors test_r5_fml_transforms.py with R4 field shapes (matchbox_scripts#8):
  Encounter.class as single Coding, not CodeableConcept[]
  Encounter.period, not actualPeriod
  Encounter.hospitalization, not admission
  Procedure.performed[x], not occurrence[x]
  MedicationStatement.medicationCodeableConcept, not medication.concept

Issues:
  matchbox_scripts#8  https://github.com/croeder-fhir-to-omop/matchbox_scripts/issues/8
...
def _retrying(fn, passes, attempts=5, delay=3):
    ...
    Handles enchilada cold-cache flakiness: a valid resource is returned but
    terminology-translated fields (e.g. drug_concept_id) are absent when the
    lookup raced the vocabulary cache warming up.
    """
...
# R4: Encounter.class is a single Coding (R5 changed it to CodeableConcept[]).
# R4: Encounter.status uses "finished" (R5 renamed this value to "completed").
ENCOUNTER_AMB = {
...
ENCOUNTER_IMP = {
    ...
    "subject": {"reference": "Patient/1"},
    "period": {"start": "2020-02-01T09:00:00Z", "end": "2020-02-03T12:00:00Z"},
}
...
# R4: Procedure.performed[x] (R5 renamed this to occurrence[x]).
PROCEDURE_DATETIME = {
...
ENCOUNTER_WITH_PERIOD = {
    ...
    "subject": {"reference": "Patient/1"},
    "period": {"start": "2020-01-01T09:00:00Z", "end": "2020-01-01T10:00:00Z"},
    # R4: Encounter.hospitalization (R5 renamed this to admission).
    "hospitalization": {
...
class TestEncounterVisitMapLocal:
    ...
    def test_WHEN_encounter_has_period_SHOULD_produce_visit_start_date(self):
        ...
        assert result.get('visit_start_date') is not None, (
            f"Expected visit_start_date, got None"
        )
    ...
    def test_WHEN_encounter_has_hospitalization_SHOULD_produce_discharged_to_source_value(self):
        ...
        assert result.get('discharged_to_source_value') is not None, (
            f"Expected discharged_to_source_value, got None"
        )
...
class TestPersonMapRaceEthnicity:
    ...
    NOTE: the two "known code" tests below currently fail (translate() falls back to 0
    even when a recognized OMB code is present) — see matchbox_scripts#19. Reproduces
    identically on the r5 stack, so it isn't an R4/R5 shape issue.
    """
...
class TestProcedureMap:
    ...
    def test_WHEN_procedure_has_performedDateTime_SHOULD_produce_procedure_date(self):
        ...
        assert result.get('procedure_date') is not None, (
            f"Expected non-null procedure_date, got None"
        )
...
# R4: MedicationStatement.medicationCodeableConcept (R5 renamed this to medication.concept).
# R4: MedicationStatement.status uses "active" (R5's "recorded" is not an R4 status code).
MEDICATION_ASPIRIN = {
...
# R4: MedicationRequest.medicationCodeableConcept (R5 renamed this to medication.concept).
MEDICATION_REQUEST_METFORMIN = {
...
class TestMedicationStatementMap:
    """MedicationMap — MedicationStatement → OMOP DrugExposure table.

    Verifies that the map produces a valid DrugExposure row with stable IDs,
    correct date fields, and a resolved drug_concept_id/drug_source_value from
    ...
    def test_WHEN_medication_is_transformed_SHOULD_produce_drug_concept_id(self):
        result = transform(MEDICATION_ASPIRIN, 'MedicationMap')
        assert result is not None
        assert result.get('drug_concept_id') is not None, (
            'Expected drug_concept_id from medicationCodeableConcept.coding translate()'
        )

    def test_WHEN_medication_is_transformed_SHOULD_have_drug_source_value(self):
        result = transform(MEDICATION_ASPIRIN, 'MedicationMap')
        assert result is not None
...
@pytest.mark.skip(reason="No MedicationRequestMap exists in IG 1.0.0/1.0.1 yet")
class TestMedicationRequestMap:
    """MedicationRequestMap handles FHIR R4 MedicationRequest.medicationCodeableConcept."""

    def test_WHEN_medication_request_transformed_SHOULD_return_drug_exposure(self):
        result = transform(MEDICATION_REQUEST_METFORMIN, 'MedicationRequestMap')
        assert result is not None, (
            'MedicationRequestMap returned OperationOutcome — map may be missing'
        )

    def test_WHEN_medication_request_transformed_SHOULD_produce_drug_concept_id(self):
        result = transform(MEDICATION_REQUEST_METFORMIN, 'MedicationRequestMap')
        assert result is not None
        assert result.get('drug_concept_id') is not None, (
            'Expected drug_concept_id from medicationCodeableConcept.coding translate()'
    ...
    def test_WHEN_medication_request_transformed_SHOULD_produce_start_date(self):
        result = transform(MEDICATION_REQUEST_METFORMIN, 'MedicationRequestMap')
        assert result is not None
        assert result.get('drug_exposure_start_date') is not None

    def test_WHEN_medication_request_transformed_SHOULD_have_drug_source_value(self):
        result = transform(MEDICATION_REQUEST_METFORMIN, 'MedicationRequestMap')
        assert result is not None
        src = result.get('drug_source_value')
...
class TestConditionMap:
    """ConditionMap — Condition → OMOP ConditionOccurrence table."""

    def test_WHEN_condition_transformed_SHOULD_return_condition_occurrence(self):
        result = transform(CONDITION_FEVER, 'ConditionMap')
        assert result is not None, 'ConditionMap returned OperationOutcome'

    def test_WHEN_condition_transformed_SHOULD_have_condition_concept_id(self):
        result = transform(CONDITION_FEVER, 'ConditionMap')
        assert result is not None
        cid = result.get('condition_concept_id')
        assert cid is not None, 'Expected condition_concept_id from code.coding translate()'

    def test_WHEN_condition_transformed_SHOULD_have_start_date(self):
        result = transform(CONDITION_FEVER, 'ConditionMap')
        assert result is not None
        assert result.get('condition_start_date') is not None

    def test_WHEN_condition_transformed_SHOULD_have_type_concept_id_32817(self):
        result = transform(CONDITION_FEVER, 'ConditionMap')
        assert result is not None
