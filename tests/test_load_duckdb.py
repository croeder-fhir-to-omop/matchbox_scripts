...
class TestWriteReportCsvSection:
    """write_report should include a CSV output section when csv_rows is provided.

    Issue: ETL report had no links to the generated OMOP CSV files even though
    they are served at http://localhost:8088/csv/.
    """

    def test_WHEN_csv_rows_given_SHOULD_include_csv_table_heading(self):
        html = _call_write_report(SAMPLE_RESULTS, SAMPLE_CSV_ROWS)
        assert 'Output Tables' in html, (
            'Expected a CSV output section heading, not found in report HTML'
        )

    def test_WHEN_csv_rows_given_SHOULD_include_link_to_person_csv(self):
        html = _call_write_report(SAMPLE_RESULTS, SAMPLE_CSV_ROWS)
        assert 'csv/person.csv' in html, (
            'Expected href to csv/person.csv in report HTML'
        )
    ...
    def test_WHEN_csv_rows_given_SHOULD_include_link_to_condition_occurrence_csv(self):
        ...
        assert 'csv/condition_occurrence.csv' in html, (
            'Expected href to csv/condition_occurrence.csv in report HTML'
        )

    def test_WHEN_csv_rows_given_SHOULD_show_row_count_for_person(self):
        html = _call_write_report(SAMPLE_RESULTS, SAMPLE_CSV_ROWS)
        # person has 2 rows
        assert '>2<' in html or '>2 ' in html or '2 row' in html, (
            'Expected row count 2 for person table in report HTML'
        )

    def test_WHEN_csv_rows_given_SHOULD_show_row_count_for_condition_occurrence(self):
        html = _call_write_report(SAMPLE_RESULTS, SAMPLE_CSV_ROWS)
        # condition_occurrence has 1 row
        assert '>1<' in html or '1 row' in html, (
            'Expected row count 1 for condition_occurrence table in report HTML'
        )
    ...
    def test_WHEN_csv_rows_is_none_SHOULD_not_include_csv_section(self):
        html = _call_write_report(SAMPLE_RESULTS, None)
